package com.example.fathead.fathedweather.service;
...
public class AutoUpdateService extends Service {
    ...
    public  int onStartCommand(Intent intent,int flags,int startId){
        ...
        //获取AlarmManager的实例（涉及到定时任务ALARM机制的用法）
        int anHour=8*60*60*1000;
        //8小时毫秒数
        long triggerAtTime= SystemClock.elapsedRealtime()+anHour;
        //elapsedRealtime()可以获取系统开机至今所经历的毫秒
        Intent i = new Intent(this, AutoUpdateService.class);
        PendingIntent pi=PendingIntent.getService(this,0,i,0);
        //调用getService获得能够执行服务的PendingIntent
        manager.cancel(pi);
        manager.set(AlarmManager.ELAPSED_REALTIME_WAKEUP,triggerAtTime,pi);
        //ELAPSED_REALTIME_WAKEUP指定AlarmManager的工作类型
        //triggerAtTime为定时任务触发的时间
        return super.onStartCommand(intent,flags,startId);
        //定时任务被触发，服务的onStartCommand得到执行
